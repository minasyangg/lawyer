    ...
    rl.question('', async (answer) => {
      ...
      for (const update of updates) {
        try {
          ...
          console.error(`❌ Ошибка обновления папки "${update.oldName}":`, errorMessage)
        }
      }
      
      console.log('\n✨ Готово!')
      console.log('\n⚠️  ВАЖНО: Вам нужно также переименовать папки в Supabase Storage вручную или через админ-панель')
      console.log('💡 Используйте файловый менеджер для переименования папок в облачном хранилище')
